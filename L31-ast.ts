// ===========================================================
// AST type models
import { all, any, map, zipWith } from "ramda";
...
export const makeLitExp = (val: SExpValue): LitExp =>
    ({tag: "LitExp", val: val});
export const makeClassExp: (fields: VarDecl[], methods: Binding[]) => ClassExp = (fields: VarDecl[], methods: Binding[]): ClassExp => 
...
const parseClassExp = (fields: Sexp, methods: Sexp[]): Result<ClassExp> => {
    const classMethods: Sexp = methods[0];
    if(isArray(fields) && allT(isIdentifier, fields) && allT(isString, fields) && isGoodBindings(classMethods)){
        const goodFields: VarDecl[] = map(makeVarDecl, fields);
        const methodBodies: Result<CExp[]> = mapResult((classMethod: [string, Sexp]) => parseL31CExp(second(classMethod)), classMethods);
        if(isOk(methodBodies) && !allT(isProcExp, methodBodies.value))
            return makeFailure("Class methods not defined correctly!");
        const methodNames: Sexp[] = map((classMethod: [string, Sexp]) => (first(classMethod)), classMethods)
        if(allT(isString, methodNames)){
            const goodMethods: Result<Binding[]> = bind(methodBodies, (bodies: CExp[]) => makeOk(zipWith(makeBinding, methodNames, bodies)));
            return bind(goodMethods, (goodMethods: Binding[]) => makeOk(makeClassExp(goodFields, goodMethods)));
        }
        return makeFailure("Class method names not defined correctly!")
    }
    return makeFailure("Wrong class syntax!");
}
...
const unparseClassExp = (ce: ClassExp) : string => 
    `(class (${map((field: VarDecl) => `${field.var}`, ce.fields).join(" ")}) (${map(unparseBindings, ce.methods).join(' ')}))`

const unparseBindings: (b: Binding) => string = (b: Binding): string =>
        '('+b.var.var+' '+unparseL31(b.val)+')';
